document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayPosts(posts) {
    ...
    posts.forEach((post) => {
      ...
      title.style.cursor = "pointer"; // Делаем курсор указателем при наведении
      title.addEventListener("click", () => {
        window.location.href = `post-details.html?postId=${post.id}`; // Перенаправление на страницу деталей поста
      });
